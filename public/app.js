...
/* ---------------- Estado ---------------- */
const App = { stack: [], current: { screen: 'home', params: {} }, ctx: {} };
...
function centerCard(c, mode) {
  ...
  if (mode === 'donate') {
    ...
    buttons = `<div class="btn-row mt-16">
      ...
      <button class="btn sm ghost" data-action="share" data-id="${c.id}">${icon('share')}Compartir</button></div>`;
  }
...
screens.home = () => {
  ...
  // Resumen en vivo (centros de acopio) — se carga al iniciar.
  ...
  const homeStats = `
    <div class="stat-grid home-stats">
      <button class="stat" ${nav('centers-all')} style="grid-column:1/-1"><div class="num">${fmtN(m && m.centers && m.centers.total)}</div><div class="lab">Centros de acopio</div></button>
...
function disclaimer() {
  return `<div class="disclaimer">
    <b>AyudaVE — iniciativa sin fines de lucro.</b> La información de centros de acopio la aportan los propios usuarios y se recopila de forma automática.
...
screens['admin'] = () => {
  ...
  const data = App._admin || { pendientes: [], totals: {} };
  return { tint: '#0a2f72', title: 'Panel admin', html: `
    <div class="screen-head"><h1>Panel de administrador</h1><p class="sub">Hola, ${u.nombre}. Verifica centros de acopio.</p></div>
    <div class="stat-grid">
    ...
    <div class="section-label mt-16">Centros por verificar (${data.pendientes.length})</div>
...
screens['dashboard'] = () => {
  ...
  const pend = d.pendientes || [], rc = (d.recent && d.recent.centers) || [];
...
const actions = {
  ...
  async 'open-admin'() { try { App._admin = await API.adminOverview(); go('admin'); } catch { toast('Acceso solo para administradores', false); } },
  ...
  'help-way'(t) { App.ctx.way = t.dataset.k; go('help-reco'); },
...
window.AY_SHARE_MSG = '🆘 Por favor difundir 🇻🇪\nAyudaVE: centros de acopio, donaciones e información que necesitas tras el sismo en Venezuela.';
...
async function boot() {
  ...
  await Promise.all([refreshCenters(), refreshMetrics()]);
  render();
